...
class RrRangeService : public RrStateServiceBase
{
 public:
   ...
   * 
   * @param request, the range sensor to look for if override is set to false, or it will update or create a new range
   * @param response, the range found in the request, or the range that has just been set.
   * 
   * When called depending on what state_override is set too (either true or false), the algorithm will check for existing
   * ranges with the same frame id as the one in the request. note that only one range sensor with a specific frame_id may
   * be set,  if a request with the same frame_id is sent again and state_override is set to true, then that range will be 
   * overridden. 
   */
